...
class ListFragment :
    ...
    private fun render(state: ListContract.State) {
        ...
        // 中间的提示：空列表 或 整页失败
        val centerTip = when {
            state.empty -> getString(R.string.list_empty)
            state.failed -> state.failMessage ?: getString(R.string.list_failed)
            else -> null
        }
