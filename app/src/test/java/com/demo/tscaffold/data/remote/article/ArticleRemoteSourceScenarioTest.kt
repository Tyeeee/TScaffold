...
class ArticleRemoteSourceScenarioTest {
    ...
    @Test
    fun `业务失败但后端没给提示：提示里带上错误码，不糊弄用户`() = runBlocking {
        server.enqueue(json("""{"code":500}"""))

        val error = catchApiError { source.search("标题") }

        assertTrue("实际是 ${error.javaClass.name}", error is ApiException.Business)
        assertTrue("提示里要带上错误码，实际是：${error.message}", error.message!!.contains("500"))
    }

    @Test
    fun `说成功却没有数据：算失败，不能当空列表糊过去`() = runBlocking {
        server.enqueue(json("""{"code":0,"data":null}"""))
