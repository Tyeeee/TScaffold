package com.tscaffold.business.basic.login.contract
 ...
 * 登录页的状态和操作。
 *
 * 这一页的重点在两处，都是表单类页面最容易写错的地方：
 *
 * 1. **错误提示是算出来的**（见 [State.usernameError]），不是另存一份。
 *    少存一份，就不会出现"改了输入框、错误提示没跟着变"的经典 bug。
 * 2. **"能不能点提交"也是算出来的**（[State.canSubmit]）：正在提交时自动变灰，
 *    用户连点两下也不会提交两次。
 */
class LoginContract {
